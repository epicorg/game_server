...
public class Coordinate2D {
	...
	public Coordinate2D(Maze maze, int x, int y) {
		...
		if (x < 0 || x >= maze.getxSize() || y < 0 || y >= maze.getySize())
	...
	public Coordinate2D plusX() {
		return x == maze.getxSize() - 1 ? null : new Coordinate2D(maze, x + 1,
				y);
	};
	...
	public Coordinate2D plusY() {
		return y == maze.getySize() - 1 ? null : new Coordinate2D(maze, x,
				y + 1);
	};
