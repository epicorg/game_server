...
public class MapJSONizer {
     ...
     */
    public static ArrayList<MapDimension> getAdaptedWinPoint(ArrayList<MapDimension> winPointsToAdapt) {

        ArrayList<MapDimension> winPoints = new ArrayList<>(winPointsToAdapt);

        if (PHASE != 0)
            winPoints.replaceAll(mapDimension -> new MapDimension(mapDimension.getWidth() + PHASE,
                    mapDimension.getHeight(), mapDimension.getLength() + PHASE));
    ...
    public String getAdaptedWidth() {
        return valueOf(map.getMapSize().getWidth()).toString();
    }

    public String getAdaptedHeight() {
        return valueOf(map.getMapSize().getHeight()).toString();
    }
