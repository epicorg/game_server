...
import check_fields.RequestFieldChecker;
...
/**
 * A client dedicated thread that listen for client request and run server Services for him.
 *
 * @author Noris
 * @date 2015/03/26
 */
public class ClientRequestThread implements Runnable {
    ...
    private RequestFieldChecker checker;
    ...
    public ClientRequestThread(Socket socket) {
        ...
        checker = new RequestFieldChecker();
    }
    ...
    public void run() {
        try {
            ...
            while (!socket.isClosed()) {
                ...
                if (checker.checkRequest(jsonRequest)) {
    ...
    private JSONObject elaborateRequest(JSONObject jsonRequest) throws JSONException {
        ...
        return service.start(jsonRequest);
    }
