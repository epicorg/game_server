...
public class SecureConnectionApplicator {
	...
	/**
	 * Decrypts a request from the client. It doesn't decrypt the request if [a]
	 * the message is an encryption establishment message (the first messages of
	 * any connection), in this case it saves the wrapped symmetric key, or [b]
	 * the server is set to accept also the unencrypted requests or [c] the
	 * symmetric key is not set. In this last case the server not read the
	 * request and returns an unknown service response.
	 * 
	 * @param request
	...
	/**
	 * Encrypts the response generated by the server. It doesn't encrypt the
	 * response if [a] the message is an encryption establishment message or [b]
